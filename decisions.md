...
**Prepend is a two-phase coordinate transition**: `setData()` invalidates the time-to-index mapping, so only `setVisibleLogicalRange()` works immediately after (LWC defers reflow ~50ms). `setVisibleRange()` is a no-op in this window. Phase 1: `maintainScrollPosition` shifts the logical range and sets `needsEnforce = true` if tracking. Phase 2: after grace frames expire (LWC has reflowed), `updateAnchor` sees `needsEnforce` and fires `enforceLivePosition()` in time-space — re-establishing the coordinate system the drift checker reads from. This separates the "keep bars stable" concern (immediate, bar-index-space) from "keep drift checker sane" concern (deferred, time-space).
